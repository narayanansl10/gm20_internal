...
let smtpTransport = nodemailer.createTransport({
    service: "gmail",
    auth: {
        user: "gyanmitra20@gmail.com",//Have to be created
        pass: "gyan94860"
    }
});
                        ...
                        if (count == 0) {
                            ...
                            PasswordToken.create({
                                ...
                                if (error) {
                                    ...
                                    link = "//URL here" + "resetPassword/" + token;
                                    let mailOptions = {
...
router.get('/participants/search', (req, res, next) => {
    ...
    User.findById(_id).populate('department_id').populate('year_id').exec((err, docs) => {
        if (!err) {
            res.json(docs);
        } else {
            res.json({
...
router.get('/participants', function (req, res, next) {
    ...
    if (!req.query.page) {
        User.find({
            type: 'user'
        }).populate('department_id').populate('year_id').exec(function (err, docs) {
            if (!err) {
        ...
        User.find({}).skip(config.pagination.perPage * (page - 1)).limit(config.pagination.perPage).exec((err, docs) => {
            if (!err) {
                res.send(docs);
            } else { }
...
router.post('/participants/filter', function (req, res) {
    User.find({
        type: 'user',
        rollo: {
            $regex: req.body.rollno
        },
        Gender: {
            $regex: req.body.gender
        },
    }).populate('rollno').populate('department_id').populate('year_id').exec(function (err, docs) {
        if (!err) {
            res.send(docs);
        } else {
            res.send({
...
//Reviewed and Modified by Narayanan SL
//On 11-12-19